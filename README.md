...
次に、グローバル変数`cowsay`と、型定義を紐づけを行います(VSCodeがわかるように)
...
※jsファイルが多くて、コメントを挿入するのが面倒な場合、TypeScriptを導入し型チェック機能だけを利用する方法もあります
...
`アンビエント宣言`は、変数、関数が「存在する」ことをコンパイラに伝える機能です。ここではグローバル変数`cowsay`がどこかに存在する(ので未定義エラーにしないでねと伝える)ことと、その型定義がどこにあるか？を記載します
